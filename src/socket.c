...
struct us_socket_t *us_socket_attach(int ssl, LIBUS_SOCKET_DESCRIPTOR client_fd, struct us_socket_context_t *ctx) {
    struct us_poll_t *accepted_p = us_create_poll(ctx->loop, 0, sizeof(struct us_socket_t) - sizeof(struct us_poll_t) + sizeof(size_t));
    us_poll_init(accepted_p, client_fd, POLL_TYPE_SOCKET);
    us_poll_start(accepted_p, ctx->loop, LIBUS_SOCKET_READABLE);
